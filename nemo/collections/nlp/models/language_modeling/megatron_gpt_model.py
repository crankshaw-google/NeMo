...
class MegatronGPTExportableModel(torch.nn.Module, Exportable):
  ...
  def forward(self, tokens, position_ids, attention_mask):
    if self.fp8_enabled and HAVE_TE:
      with transformer_engine.pytorch.onnx_export(
          self.fp8_enabled
  ...
  def input_example(self, max_batch=1, max_dim=768, seq_len=6):
    ...
    for tokens, attn_mask_and_pos_ids in zip(
        id_tensors, masks_and_position_ids
    ):
...
class MegatronGPTModel(MegatronBaseModel, TextGeneration):
  ...
  def __init__(self, cfg: DictConfig, trainer: Trainer, profiler=None):
    if not HAVE_APEX:
      raise ImportError(
          'Apex was not found. Please see the NeMo README for installation'
          ' instructions: https://github.com/NVIDIA/NeMo#megatron-gpt.'
      )
    ...
    self.tokenizer = None
    self._pytorch_profiler = profiler
    ...
    if not self.megatron_amp_o2 and self.cfg.get(
      ...
      raise ValueError(
          'Virtual pipeline model parallel is only supported when using'
          ' megatron_amp_O2'
      )
      ...
      if isinstance(self.model, list):
        ...
        for module in self.model:
          converted_model.append(
              Float16Module(module=module, precision=cfg.precision)
          )
  ...
  def on_train_batch_start(self, outputs, batch, batch_idx):
    super().on_train_batch_start(outputs, batch, batch_idx)
  ...
  def training_step(self, dataloader_iter, batch_idx):
    ...
    if self.cfg.get('rampup_batch_size', None):
      ...
      num_microbatch_calculator = (
          apex.transformer.pipeline_parallel.utils._GLOBAL_NUM_MICROBATCHES_CALCULATOR
      )
  ...
  def backward(self, *args, **kwargs):
    ...
    We want this to do nothing since we run backward in the fwd/bwd functions
    from megatron-core. No need to call it here.
    """
    return
  ...
  def validation_step(self, dataloader_iter, batch_idx):
    ...
    a number of microbatches depending on the global batch size and model
    parallel size
    from the dataloader to produce a list of microbatches.
    The list of microbatches is then piped through the pipeline using
    megatron-core fwd/bwd functions.
    """
    # Initialize userbuffer communicators.
    if self.initialize_ub:
      self.initialize_ub_func()
  ...
  def build_train_valid_test_datasets(self):
    ...
    if self.trainer.limit_val_batches > 1.0 and isinstance(
      ...
      raise ValueError(
          'limit_val_batches must be an integer or float less than or equal to'
          ' 1.0.'
      )
  ...
  def setup(self, stage=None):
    ...
    Args:
        stage (str, optional): Can be 'fit', 'validate', 'test' or 'predict'.
          Defaults to None.
    """
    num_parameters_on_device, total_num_parameters = (
        self._get_total_params_across_model_parallel_groups_gpt_bert(self.model)
    )

    logging.info(
        'Pipeline model parallel rank:'
        f' {parallel_state.get_pipeline_model_parallel_rank()}, Tensor model'
        f' parallel rank: {parallel_state.get_tensor_model_parallel_rank()},'
        ' Number of model parameters on device:'
        f' {num_parameters_on_device:.2e}. Total number of model parameters:'
        f' {total_num_parameters:.2e}.'
    )
    ...
    if rampup_batch_size:
      ...
      assert (
          ...
          'expected batch size increment ({}) to be divisible by'
          ' micro_batch_size ({}) times total data parallel size ({})'.format(
              batch_size_increment, micro_batch_size, total_data_parallel_size
          )
      )
  ...
  def setup_validation_data(self, cfg):
    if hasattr(self, '_validation_ds'):
      consumed_samples = 0
      logging.info(
          'Setting up validation dataloader with'
          f' len(len(self._validation_ds)): {len(self._validation_ds)} and'
  ...
  def transfer_batch_to_device(
    ...
    When using pipeline parallelism, we need the global batch to remain on the
    CPU,
    since the memory overhead will be too high when using a large number of
    microbatches.
  ...
  def _validate_trainer(self):
    ...
    if self.trainer.accumulate_grad_batches > 1:
      raise ValueError(
          f'Gradient accumulation is done within training_step.'
          f' trainer.accumulate_grad_batches must equal 1'
      )
  ...
  def _reset_activation_checkpointing_args(self):
    ...
    _restore_activation_checkpointing_args can restore them later. This function
    must always be
  ...
  def _restore_activation_checkpointing_args(self):
    ...
    _reset_activation_checkpointing_args. This function must never be called
    before
  ...
  def _reset_sequence_parallelism_args(self):
    ...
    _restore_sequence_parallelism_args can restore them later. This function
    must always be
